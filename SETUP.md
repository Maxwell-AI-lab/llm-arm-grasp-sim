...
关键设计：LLM **看不到关节、不知道 IK**，每步只看 2 路相机图（顶视 + 斜侧视）和末端坐标，
...
- 相机：`top`、`side` 两路 448×448（见 §4.5 的配置说明）
...
- `top`：工作台正上方俯视，提供方块的 x-y 平面位置
- `side`：从前方（-y 侧）斜视，提供高度（z）与前后（x）关系

**与官方配置的差距（必须声明）**：YAM 官方真机有 3 路相机——顶视（D435）
+ 左右腕部各一路（D405）。腕部相机正对钳口，是最后 5cm 精细对中的关键视角；
本环境没有腕部相机，LLM 在最终逼近阶段只能靠俯视和斜视估计，任务难度高于官方设置。
所有已完成试验（K3 等）均在此两路配置下进行，跨模型对比时保持此配置不变即可保证公平。

如后续想对齐官方配置，可在腕部（`link_6`）挂载一路相机，每帧按夹爪位姿更新
相机世界位姿（lookat），并在 `info.observation_space.cameras` 与 `_observe()`
中同步增加一路渲染；改动后必须重跑 §5 的全知对照实验。
...
4. 相机仅顶视 + 斜侧视两路；官方为顶视 + 左右腕部共 3 路（见 §4.5）
5. 单臂、腕部姿态固定（无 yaw/pitch/roll 控制）；官方双臂 6-DoF
6. PD 伺服加强过（官方真机伺服参数不可知）
7. 仿真分数 ≠ 真机分数；同仿真内跨模型横向对比有效
